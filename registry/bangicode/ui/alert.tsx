...
const alertVariants = cva(
  [
    "relative w-full rounded-sm border p-4",
  ...
  ].join(" "),
  {
    variants: {
      variant: {
        ...
        info: "border-secondary bg-secondary/10 text-foreground [&>svg]:text-accent",
...
const AlertDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("font-hanken-grotesk text-sm [&_p]:leading-relaxed", className)}
      {...props}
    />
  ),
);
